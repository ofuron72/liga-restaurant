...
class KitchenOrderEntityDtoToResponseMapperTest {
    ...
    void testMapDtoToResponse_shouldMapCorrectly() {
        ...
        assertEquals(dto.getWaiterOrderNo(), response.waiterOrderNo());
        assertEquals(dto.getStatus(), response.status());
        assertEquals(dto.getOrderIdWaiterService(), response.orderIdWaiterService());
    }
}